...
from luq_model import LuqIDSModel
...
# model = LuqIDSModel(input_features=train.shape[1]-1, num_classes=len(config['class_names'])).to(device)
...
def train_model(model, train_loader, val_loader, device, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), 'luq_truthseeker.pth')
            print(f"Model saved to IDS_CICIDS2017.pth")
            no_improve = 0
        else:
            no_improve += 1
    ...
    model.load_state_dict(torch.load('luq_truthseeker.pth', map_location=device))
    return model
...
def test_and_report(model, test_loader, device, class_names):
    ...
    # If the provided class_names list is longer (e.g. full 15 classes) but the
    # test set only contains a subset (e.g. 2 classes), align the names with
    # the labels that are actually present to avoid sklearn's size mismatch error.
...
if __name__ == "__main__":
    ...
    wandb.init(project="TruthSeeker",
               config={
